...
export function BookingFormImproved({ services, onSubmitSuccess, onSubmitError }: BookingFormProps) {
  ...
  async function onSubmit(values: BookingFormValues) {
    ...
    try {
      ...
      // Add hidden inputs for all form values
      const formData = new FormData(formElement)

      // Add date in formatted string
      if (values.date) {
        formData.set("date", format(values.date, "PPP", { locale: ro }))
      }
      ...
      // Add recipient email
      formData.set("to_email", "eliezer.cleaning.sm@gmail.com")

      // Send the form using EmailJS
      await emailjs.sendForm("service_m3w0ype", "template_lpflh8e", formElement, "dwErPR86QGQa-LJhj")
